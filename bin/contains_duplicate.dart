...
bool containsDuplicate(List<int> nums) {
  ...
  for (int i = 0; i < nums.length; i++) {
    // jika gagal menambahkan data ke Set, berarti ada duplicate
    if (!seen.add(i)) return true;
    print('iterasi ${nums[i]} = ${!seen.add(i)}');
  }

  return false;
}

bool containsDuplicateDua(List<int> nums) {
  Set<int> seen = {}; // Set untuk menyimpan elemen unik

  for (int num in nums) {
    if (seen.contains(num)) {
      return true; // Jika elemen sudah ada, kembalikan true
    }
...
String hasilString(List<int> nums) {
  bool isDuplicate = containsDuplicate(nums);

  if (isDuplicate == true) {
    return 'Ada data yang duplicate';
  }
  return 'Semua data adalah unik';
}

void main() {
  List<int> nums = [1, 3, 5, 6, 5, 7, 8];
  // var cariDuplicat = containsDuplicate(nums);
  // print(cariDuplicat);
  var cariDuplicat2 = containsDuplicateDua(nums);
  print(cariDuplicat2);

  // String hasilPantau = hasilString(nums);
  // print(hasilPantau);
}
